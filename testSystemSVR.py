...
if __name__ == '__main__':
    ...
    zIdx = system.bus.vm
    zUb = 1.1 * np.ones(system.bus.n)
    zLb = 0.9 * np.ones(system.bus.n)
    # ----------------------------------------------------------------------------------
  

    # Control!!!
    buspilot = [5, 6, 8] #Nombre de los nodos piloto.
    avrIdx = [0,1,2] #Indice correspondiente a vref en el DAE.
    qhIdx = [25, 33, 41] #Indice correspondiente a la  potencia reactiva inyectada por los generadores en el DAE.
    #Algebraic variables of interest
    zIdx = [1,3,5]
    zUb =  1.1 * np.ones(len(zIdx))
    zLb =  0.9 * np.ones(len(zIdx))
    alpha = alpha = (system.syn4.pgen ** 2 + system.syn4.qgen ** 2) ** 0.5 / ((system.syn4.pgen ** 2 + system.syn4.qgen ** 2) ** 0.5).sum()
    svr = svrSA(dae, system, buspilot, avrIdx, qhIdx, alpha)
    ...
    t1 = time.time()
    # dSimulation(system, dae, tMax=tMax, dT = 250e-3, event=event, control=None)
    cSimulation(system, dae, tMax = tMax, dT = dT,  iterMax = 10, tol = 1e-4, event = event, control= svr)
    ...
    # axs[0,1].plot(agent.tOut, agent.yrOut)
    plt.show()
